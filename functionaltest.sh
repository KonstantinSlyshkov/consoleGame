...
while true
do
OUTCOME=$(curl -s "https://circleci.com/api/v1.1/project/github/ConstellationBrands/KonstantinSlyshkov/getftpfeedswithnotifications/${BUILD_NUMBER}?circle-token=${token}" | sed -n 's/\"outcome\"[[:space:]]:[[:space:]]\"//p' | sed s'/\",$//' | tail -n 1)
